<div class="cover-page">
    <h1>タイトル: X Window Systemのキー割り当ての設定</h1>
    <h2>サブタイトル: </h2>
  <div class="info">2025年12月24日</div>
  <!-- <div class="info">この書き方で内容を適宜追加できます</div> -->
</div>
<br>

# X Window Systemのキー割り当て

X Window Systemでは、キーとマウスのボタンに対する意味の割り当てをサーバが管理しています。この管理情報を変更することで、キーあるいはマウスのボタンの割り当てを変更することができます。キーあるいはマウスのボタンの割り当ての変更には、xmodmapというコマンドを使います。

キーボード上の各キーは、キーコードによって特定できます。キーコードは、あるキーを押した時にハードウェア内部で発生する、ハードウェア依存の数値です。
これに対して、キーの機能と位置にしたがって各キーに付けられた名前がKeySym名です。キーコードとKeySym名の対応はXサーバによって管理されているので、標準的に用意されている範囲のキーであれば、違うハードウェアであっても同じKeySym名を使うことができます。例えば、通常、Deleteには"Delete"というKeySym名が、左のShiftキーには"Shift_L"というKeySym名が割り当てられています。通常の文字キーにも、入力する文字に応じて"a"とか"percent"といったKeySym名が割り当てられています。

## 現在の割り当て状態の取得

以下のオプションをxmodmapに与えることで、現在のキーとマウスのボタンの割り当て状態を調べることができます。

|オプション|意味|
|---|---|
|-pk|現在のキーの割り当てを出力する|
|-pm|現在の修飾キーの割り当てを出力する|
|-pp|現在のマウスボタンの割り当てを出力する|
