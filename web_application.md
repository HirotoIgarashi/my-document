...
## フォーム
Webアプリケーションにおいてフォームの定義についてウィキペディアから引用します。[フォーム(ウェブ)](https://ja.wikipedia.org/wiki/%E3%83%95%E3%82%A9%E3%83%BC%E3%83%A0_%28%E3%82%A6%E3%82%A7%E3%83%96%29)
フォーム（英: Form）、入力フォームは、Webプログラミングの技術の1つ。クライアントが情報を入力・選択し、Webサーバ等のフォームを処理するエージェントへ提出するための機構である。とあります。

ユーザがWebアプリケーションにデータを入力し、サブミットボタンなどで提出(サブミット)すると理解しています。

HTMLの基本的な記述は以下のようになるでしょう。
~~~
<form>
  <input type="text">
  <button>送信</button>
...
![ボックス](https://snsd.mydns.jp/markdown_image/css_box.png)
...
### Grid Layout Module
Grid Layoutの背後にある中心的な概念は、Webページを列と行に分割し、作成した列と行のサイズや、レイヤーの定義に基づいて構成要素の位置やサイズを決定できるというものです。

グリッドを使えば、HTMLをまったく変更せずにCSSだけで要素の位置を柔軟に変更できます。メディアクエリと併用すれば、さまざまなブレークポイントでのレイアウト変更が可能です。

Grid Layout Moduleでの主な用語とその意味は以下のとおりです。

* グリッド要素

    グリッドの境界となる要素です。グリッドの大きさはここで指定されます。
...
* グリッド線
    ...
    行や列を区切る線です。概念的なものであり、画面には表示されません。

* グリッドトラック

    「行または列」を表します。グリッドの中で行や列を作成すると、それはグリッドトラックと呼ばれます。グリッド線の間にある空間とも言えます。

* グリッド領域

    それぞれの行と列が交わる部分を指します。表でのセルのようなものです。
...
# Web API
Webは、さまざまな役に立つタスクを実行するための多種多様なAPIを提供します。これらはJavaScript APIを使用してアクセスでき、WebGLやWeb AudioといったAPIを使用して複雑なグラフィックやオーディオの効果を生成するなど、任意のwindowやelementに微調整を加えることによりどのようなことでも可能にします。
...
![BOMの構造](https://snsd.mydns.jp/markdown_image/bom_construction.png)
...
#### DOMツリー
次のDOMツリーを構成する要素はすべてNodeオブジェクトと呼ばれます。

![DOMの構造](https://snsd.mydns.jp/markdown_image/dom_construction.png)
...
#### Elementと要素の取得
ドキュメント内のすべての要素オブジェクトは、Elementから機能とプロパティの基本セットを継承しています。主な機能は、属性の値の取得(get)、設定(set)、削除(remove)、特定の属性の存在の確認(hasAttribute)です。
...
補足:書籍「初めてのJavaSctipt」では本節のタイトルは「文脈を指定した」となっているが「コンテキストを指定した」という表現のほうが理解しやすいと思います。コンテクストはアプリケーションの配置場所や呼び出し方などの情報を設定するという意味で使われることがあります。要素を取得する書き方という意味だと思っています。

要素を取得するメソッドは次のとおりです。

* Element.getElementsByClassName()

    現在の要素から、引数に指定したクラス名のリストに該当するすべての子孫要素を持つHTMLCollectionを返します。

* Element.getElementsByTagName("p")

    現在要素から、特定のタグ名を持つすべての子孫要素の集合を取得した、HTMLCollectionを返します。
* Element.getElementById("div1")

    指定されたIDを持つ要素を返します。

#### DOMツリーの変更
documentは、すべてのページ要素の親であり、オーナーでもあります。そのため、新しい要素のインスタンスを生成するために利用するファクトリメソッドのほとんどは、Coreのdocumentオブジェクトに属します。それに対して、Nodeオブジェクトは、Core APIにおける木構造の整合性を保つ役割を担当し、親子関係や兄弟関係をたどって、特定のノードに到達することを可能にしています。そして、Elementオブジェクトは、入れ子になった要素の中の、限られた範囲にあるものだけを変更するために、コンテキストを制限する方法を提供します。これらの3つのオブジェクトは、いずれもDOMツリーの変更には欠かせないものとなっています。
...
|createAttribute(name)    |Attr                   |Elementの属性                  |
...
#### DOMアクセス
DOMアクセスはコストがかかります。DOMアクセスを減らすには以下の指針があります。

* DOMアクセスのループを避ける。
* DOM参照をローカル変数に代入して、そのローカル変数で作業する。
* 可能ならばセレクタAPIを使う。
* HTMLコレクションを反復処理するときはlengthをキャッシュする。

~~~
// アンチパターン
for (var i = 0; i < 100; 1 += 1) {
...
次のコードでは(ローカル変数styleを使っている)2番目のやり方は、1行多く、変数も多いですが、こちらの方が優れています。
...
セレクタAPIを使うには次のメソッドを使います。
~~~
document.querySelector( "ul .selected" );
document.querySelectorAll( #widget .class" );
~~~

#### ドキュメントフラグメントを使用したDOM操作
参考文献: JavaScriptパターン

DOM要素へのアクセス以外に、変更、削除、追加も必要になります。DOMを更新すると、ブラウザは画面の再描画を行い、また要素のジオメトリーが再計算(リフロー)されることもあるので、コストがかかります。

DOMの更新はできるだけ減らすのが大原則です。変更をひとつにまとめて、生のDOMツリーの外側で実行スべきです。

比較的大きな部分ツリーを作成する必要があるときは、生のドキュメントへの追加は最後にまとめて行うべきです。このためには、ノードをすべて含むドキュメントフラグメントを使うことができます。

次のやり方でノードを追加してはいけません。
~~~
// アンチパターン
// ノードを作成するたびに追加する
...
もっと良いやり方にするには、まずドキュメントフラグメントを作成し、これを「オフライン」で更新し、準備が完了したら生のDOMに追加します。ドキュメントフラグメントをDOMツリーに追加するとき、そのフラグメントそのものではなく、フラグメントの内容が追加されます。これは非常に便利です。そのため、ノードがそれに適した親要素にふくまれていない(たとえばパラグラフがdiv要素の中にない)ときでも、たくさんのノードをドキュメントフラグメントで包むのは良い方法です。

次の例ではドキュメントフラグメントを使います。
...
~~~
この例では生のドキュメントが更新されるのは1回きりです。このためリフロー/再描画も1回きりです。これに対して前の例のアンチパターンでは、パラグラフごとに毎回更新されます。

ドキュメントフラグメントは、ツリーに新しいノードを追加するときも役に立ちますが、既存のツリーの一部を更新するときには、変更をひとつにまとめることもできます。変更したい部分ツリーのルートのクローンを作り、このクローンに対してすべての変更を行い、変更が終了したら元の部分ツリーをクローンで置き換えます。
...
![Ajaxのイメージ](https://snsd.mydns.jp/markdown_image/ajax.png)
...
![SPAアーキテクチャ](https://snsd.mydns.jp/markdown_image/spa_architecture.png)
